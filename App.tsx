...
const App: React.FC = () => {
  ...
  // Phase 5: Web Worker Reference
  const workerRef = useRef<Worker | null>(null);

  // Initialize Worker on mount
  useEffect(() => {
    // Robust Worker Initialization handling potential 'import.meta.url' issues
    try {
      let workerUrl: URL;
      
      // Check if import.meta exists and has url (Standard ES Modules)
      if (typeof import.meta !== 'undefined' && import.meta.url) {
        workerUrl = new URL('./workers/markdown.worker.ts', import.meta.url);
      } else {
        // Fallback for environments where import.meta is missing
      ...
      workerRef.current.onmessage = (e) => {
        ...
        if (type === 'success') {
          // Phase 5: Sanitize on main thread (Worker has no DOM)
      ...
      // Fallback could go here (run on main thread), but for now we log error
    }
  ...
  // Send updates to worker when input changes
  useEffect(() => {
